...
def train_LisGAN(config, model, train_loader, optimizer):
    ...
    classifier = Classifier_train(config.DEVICE, config.LISGAN.CLASSIFIER_NUM_EPOCH,
                                  ...
                                  train_loader)
    logger.info("Train classifier LISGAN model")
    classifier.fit()
    for epoch in range(config.ZSL.EPOCH):
        mean_lossD = 0
        mean_lossG = 0
        sum_loss = 0
        ...
        for i_step, data in enumerate(train_loader):
            ...
            for class_label in remap_labels(train_classes, train_classes):
                sample_idx = torch.nonzero(syn_labels == class_label, as_tuple=False).squeeze()
...
def generate_dataset_classification(train_dataset, config, net_generator):
    ...
    if config.GENERALIZED:
        ...
        dataset = {object_modalities:np.float32(np.concatenate([train_emb, unseen_train_emb,
                                                     seen_test_emb, 
    ...
    dataset = TensorDataset(torch.tensor(dataset['IMG']), torch.tensor(dataset['labels']))
    return dataset, train_indexes, test_indexes

@ZSL_MODEL_REGISTRY.register()
def LisGAN_train_procedure(
    cfg, dataset, gen_syn_data=True, save_model=False, save_dir="checkpoints",
):
    """
    Starts LisGAN model training and generates zsl_embedding dataset for
    classifier training.
    Args:
    ...
    Returns:
        model: trained model.
    """
    logger.info("Building LisGAN model")
    model = nn.ModuleDict({'discriminator':Model_discriminator(cfg.DATA.FEAT_EMB.DIM.IMG , 
                                                 cfg.DATA.FEAT_EMB.DIM.CLS_ATTR, 
                                                 cfg.LISGAN.HIDDEN_SIZE.DISCRIMINATOR),
                           'generator':Model_generator(cfg.DATA.FEAT_EMB.DIM.IMG,
                                     cfg.DATA.FEAT_EMB.DIM.CLS_ATTR, 
                                     cfg.LISGAN.HIDDEN_SIZE.GENERATOR, 
                                     cfg.LISGAN.NOISE_SIZE)})

    for net in list(model.keys()):
        model[net] = model[net].to(cfg.DEVICE)

    log_model_info(model, cfg.ZSL_MODEL_NAME)

    optimizer = {'discriminator':build_optimizer(model, cfg, "ZSL"),
             'generator':build_optimizer(model, cfg, "ZSL")}
    ...
    loss_history = train_LisGAN(cfg, model, train_loader, optimizer)
    if save_model:
        # TODO: implement model saving
        pass

    #device = torch.device('cpu')
    #load_generator = Model_generator(cfg.DATA.FEAT_EMB.DIM.IMG,
    #                                 cfg.DATA.FEAT_EMB.DIM.CLS_ATTR, 
    #                                 cfg.LISGAN.HIDDEN_SIZE.GENERATOR, 
    #                                 cfg.LISGAN.NOISE_SIZE)
    #load_generator.load_state_dict(torch.load('zeroshoteval/proc/zeroshotnet/lisgan/net_generator_dict.pth', map_location=device))

    if gen_syn_data:

        return generate_dataset_classification(dataset, cfg, model['generator'])#load_generator) 

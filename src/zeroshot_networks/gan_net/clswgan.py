...
class CLSWGAN:
    ...
    def initDefoltGenerateModel (self,syn_num=100):
        ...
        with self.g2.as_default():
            ...
            self.syn_att = tf.placeholder(tf.float32,[self.syn_num, self.attSize],name='input_attributes')
            self.noise_z1 = tf.placeholder(tf.float32,[self.syn_num, self.nz],name='noise')
            
            # model definition
            noise1 = tf.concat([self.noise_z1, self.syn_att], axis=1)
            self.genResFeatures = self.defoltGenerator( noise1,
                                            isTrainable=False,
                                            reuse=False)

...
def init_arguments():
    ...
    """
    parser = argparse.ArgumentParser(description='Main script-launcher for evaluation of ZSL models')
    ...
    parser.add_argument('--zsl-embeddings-path', required=True, 
                        help='Path to embeddings computed using ZSL models to be evaluated.')
    parser.add_argument('--zsl-train_embeddings-path', default='',
...
def check_arguments(args):
    ...
    if 'classification' in procedures:
        if args.zsl_train_embeddings_path == '':
            raise ValueError('For classification testing procedure please specify training embeddings.')

...
def init_arguments():
    ...
    saveload_args.add_argument(
        "--saved-obj-embeddings-path",
        default=default.saved_obj_embeddings_path,
        help="Path to stored object embeddings to load",
    )
    saveload_args.add_argument(
        "--obj-embeddings-save-path",
        default=default.obj_embeddings_save_path,
...
def load_configurations(args):
    return generate_config(
        parsed_model=args.model, parsed_datasets=args.datasets
    )
...
def split_dataset(modalities_dict, splits_df):
    ...
    test_unseen_df = splits_df.loc[
        (splits_df["is_train"] == 0) & (splits_df["is_seen"] == 0)
    ].astype(int)
    test_seen_df = splits_df.loc[
        (splits_df["is_train"] == 0) & (splits_df["is_seen"] == 1)
    ].astype(int)
    ...
    for dataset_name, data_array in modalities_dict.items():
        # NOTE: here is an assumption that in data_array ids are arranged
        # in ascending order in other case it can be a weak point for bugs
        train_data[dataset_name] = data_array[train_df["obj_id"].values]
        test_unseen_data[dataset_name] = data_array[
            test_unseen_df["obj_id"].values
        ]
...
def get_indice_split(splits_df):
    ...
    Args:
        splits(pandas DataFrame): dataframe with object Id, and ``is_train`` 
            ``is_seen`` binary columns.

    Returns:
        train_indice, test_seen_indice, test_unseen_indice: train, test seen
            and inseen indicies.
    """
    train_indice = splits_df[splits_df["is_train"] == 1]["obj_id"].to_numpy()
    test_seen_indice = splits_df[
        (splits_df["is_train"] == 0) & (splits_df["is_seen"] == 1)
    ]["obj_id"].to_numpy()
    test_unseen_indice = splits_df[splits_df["is_seen"] == 0][
        "obj_id"
    ].to_numpy()
...
def get_indice_for_zsl_emb(
    subset_indices, labels, classes, samples_per_class, use_train=True
):
    """
    """
    ...
    for label in classes:
        if use_train:
            class_indices = np.intersect1d(
                np.sort(subset_indices), np.where(labels == label)
            )
        else:
            class_indices = np.intersect1d(
                np.sort(subset_indices), np.where(labels == label)
...
def check_loaded_modalities(data_dict, modalities):
    for dataset_name, data in data_dict.items():
        ...
        if not modalities.issubset(keys):
            raise ValueError(
                "You have specified modalities that are not presented in dataset!\
                                \nAvaliable modalities for {} dataset: {}".format(
                    dataset_name, keys
                )
            )
...
def main():
    ...
    if not args.saved_obj_embeddings_path:
        ...
        for dataset_name, config in datasets_config.items():
            ...
            modalities_dict, labels, splits_df = load_dataset(
                dataset_name, modalities=args.modalities, path=config.path
            )
